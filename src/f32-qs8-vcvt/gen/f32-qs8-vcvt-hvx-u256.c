    ...
    const struct xnn_f32_qs8_cvt_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 256 * sizeof(float); batch -= 256 * sizeof(float)) {
    ...
    vx3 = xnn_fmadd_qf32(vx3, vscale, vmagic_bias);
    vx4 = xnn_fmadd_qf32(vx4, vscale, vmagic_bias);
    vx5 = xnn_fmadd_qf32(vx5, vscale, vmagic_bias);
    vx6 = xnn_fmadd_qf32(vx6, vscale, vmagic_bias);
    vx7 = xnn_fmadd_qf32(vx7, vscale, vmagic_bias);
