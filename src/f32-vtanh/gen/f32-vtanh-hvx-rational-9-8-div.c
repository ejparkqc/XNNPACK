    ...
    const struct xnn_f32_default_params unused_params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  for (; batch >= xnn_simd_bytes_f32; batch -= xnn_simd_bytes_f32) {
    ...
    const xnn_simd_f32_t vx2 = xnn_mul_qf32(vx, vx);

    // Evaluate the numerator polynomial p.
    xnn_simd_f32_t vp = xnn_fmadd_qf32(vx2, valpha_9, valpha_7);
    vp = xnn_fmadd_qf32(vx2, vp, valpha_5);
    vp = xnn_fmadd_qf32(vx2, vp, valpha_3);
    vp = xnn_fmadd_qf32(vx2, vp, vone);
    vp = xnn_mul_qf32(vx, vp);

    // Evaluate the denominator polynomial q.
    xnn_simd_f32_t vq = xnn_fmadd_qf32(vx2, vbeta_8, vbeta_6);
    vq = xnn_fmadd_qf32(vx2, vq, vbeta_4);
    vq = xnn_fmadd_qf32(vx2, vq, vbeta_2);
    vq = xnn_fmadd_qf32(vx2, vq, vone);
