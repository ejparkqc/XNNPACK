...
static XNN_INTRINSIC 
HVX_Vector Q6_Vqf32_convert_Vw(HVX_Vector vxl) {

    HVX_Vector vzero =  Q6_V_vzero();
    HVX_Vector minus_one = Q6_V_vsplat_R(0x80000000);
    HVX_Vector mant_neg_pow_2 = Q6_V_vsplat_R(0xc0000001);
    HVX_Vector exp_offset = Q6_V_vsplat_R(157); // 127 exp0 + 22 frac + 8 exp shift
    ...
    // Check if input element is a negative power of 2 (will become -1 after normalization)
    ql_neg_mantissa = Q6_Q_vcmp_eq_VwVw(zl_norm_mantissa, minus_one); //see if mantissa is -1
    // Obtain the exponent with qfp offset and normalization factor( 127 + (30 - normant) = 157 - normant)
    el_exponent = Q6_Vw_vsub_VwVw(exp_offset, el_exponent);
    // Correct mantissa  for negative numbers which are power of 2 (will become -1 after normalization)
    // Corrected mantissa is 0xc0000000, now adjust exponent by 1 as we have right shifted number by 1 to correct mantissa.
    // Hence choose num: 0xc0000001 when normalized number is -1
    zl_norm_mantissa = Q6_V_vmux_QVV(ql_neg_mantissa, mant_neg_pow_2, zl_norm_mantissa);
...
static XNN_INTRINSIC
HVX_Vector Q6_Vw_round_to_zero_Vw(HVX_Vector vout, HVX_Vector vxl, HVX_Vector vexp) {
...
// Converts a vector of QFloat 32-bit float elements to a vector of int32 elements.
static XNN_INTRINSIC
HVX_Vector Q6_Vw_convert_Vqf32(HVX_Vector vxl) {
